...
async function getSongs(songLink) {
  try {
    ...
    const linkarray = parseHTMLsongList(response.data, '.SongRow_center__3Vzso')
    return linkarray;
  } catch (error) {
    console.error(error);
  }
...
function parseHTMLsongList(html, target) {
  ...
  $(target).each((index, element) => {
    const songTitle = $(element).find('.SongTitle_heading__1pEGc').find('a').text();
    const artist = $(element).find('.SongEventRow_subtitle__30ZjH').find('a').text();
    if (songTitle !== '' || artist !== '') {
      const song = {
        title: songTitle,
        artist: artist
